...
void main(void)
{
    //调节noiseVector 的系数可以改善云雾效果。
    float intensity = (snoise(noiseVector * 0.5) +
                             0.5 * snoise(noiseVector * 1.00) +
                             0.25 * snoise(noiseVector * 2.00) + 1.75) / 3.5;                             
    ...
    float attenuation = dot(ES_normal, -normalize(ES_position.xyz)); 
